...
# LOCATION FILTERS
    ...
    def test_location_inexistent(self):
        f = LocationFilter("ksajdhfkjasdf", db='test')
        try:
            filteredVar = f.apply(db='test').variants
        except TypeError as ex:
            self.fail("Unhandled empty value: {}".format(str(ex)))
...
# FREQUENCY FILTERS
    ...
    def test_frequency_1kg_all_01(self):
        ...
        var = f.apply(db='test').variants
        v1kg = list(var)
        self.assertTrue(0 < len(v1kg) < self.N, "trivial test")
    ...
    def test_frequency_esp_all_04(self):
        ...
        var = f.apply(db='test').variants
        vesp = list(var)
        self.assertTrue(0 < len(vesp) < self.N, "trivial test")
        ...
        v1kg = list(FrequencyFilter('1kg','all')(0.01, '', '<=').apply(db='test').variants)
    ...
    def test_frequency_exac_all(self):
        ...
        var = f.apply(db='test').variants
        vexac = list(var)
        self.assertTrue(0 < len(vexac) < self.N, "trivial test")
    ...
    def test_frequency_max_all(self):
        ...
        var = f.apply(db='test').variants
        vmax = list(var)
        self.assertTrue(0 < len(vmax) < self.N, "trivial test")
